function [x_ok] = MotionCorrectionMARA(x,SD,k,p)
...
%%% k - k is a free parameter. We recommend k=20.
%%% p - Free paramter, it is the threshold to look for motion. We recomend
%%% p =1.2.
%%%
%%% OUTPUT: Corrected time series (free from motion artefacts).
%%%
%%%
%%% Dependencies: Script called CorrectBaselineMARA.
%%%
%%% Created by Sergio L. Novi, August 2017.
...
if exist('seg')~=0
    ...
    for Nchan=1:size(x,2)
        ...
        for nSeg=1:1:size(seg{Nchan},2)
            ...
            x_ok(t2,Nchan) = CorrectBaselineMARA(x1,x2,SD);
        end
        %    plot(x(:,Nchan));
        %     hold on
        %     plot(x_ok(:,Nchan));
