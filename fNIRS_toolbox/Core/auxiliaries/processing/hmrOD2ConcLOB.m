...
%
%
%   This function was updated in order to use the digitalized coordinates
%
function dc = hmrOD2ConcLOB( dod, SD, ppf )


% Check if exists the 3d coordinates.
% If this 
if isfield(SD,'SrcPos_3d')    
    if ~isempty(SD.SrcPos_3d) && ~isempty(SD.DetPos_3d)
        SD.DetPos = SD.DetPos_3d;
        SD.SrcPos = SD.SrcPos_3d;
    end
end
