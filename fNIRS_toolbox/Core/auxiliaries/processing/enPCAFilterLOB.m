...
function [yc, svs, nSV] = enPCAFilterLOB( y, SD, tInc, nSV )
...
if ndim==3
    ...
    for iConc = 1:2
        y = squeeze(yo(:,iConc,:));
        [rk,ma] = fillmissing(y,'linear'); % Added to deal with NaNs
        y=detrend(rk);
        c = y.' * y;
        [v,s,foo] = svd(c);
        u = y*v*inv(s);
