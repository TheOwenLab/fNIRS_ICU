% function dodTDDR = hmrMotionCorrectTDDR_adapted(dod,SD,sample_rate,opt)
%
...
% opt : 0 - Add the uncorrected high-frequency signal (original function)
%       1 - Does not add the high-frequency component (included by Sergio 
%       Novi)
%
% OUTPUTS:
...
%
% Small adaptation from Sergio Novi which adds an option for 
% not adding the uncorrected high frequency to the signal
...
for ii=1:length(lstAct)
    ...
    if opt == 0
        ...
        dodTDDR(:,idx_ch) = signal_low_corrected;
    end
end
